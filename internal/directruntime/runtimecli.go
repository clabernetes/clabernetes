...
var runtimeCLINames = []string{ //nolint:gochecknoglobals // fixed shim identities.
	"docker",
	"podman",
}
...
func IsRuntimeCLIInvocation(argument string) bool {
	...
	return slices.Contains(runtimeCLINames, base)
}
...
func RunRuntimeCLIShim(arguments []string) error {
	if len(arguments) < 2 || arguments[1] != "exec" {
		return errors.New("runtime CLI shim realizes only application-local exec sessions")
	}
	...
	if len(rest) < 2 {
		return errors.New("runtime CLI exec requires a container and a command")
	}
...
func prepareImportedRuntimeCLI(plan clabernetesinternaldeviceplan.Plan, containerID string) {
	runtimeID := ""
	...
	if runtimeID != "" {
		_ = os.Setenv(RuntimeExecTargetEnvironmentVariable, runtimeID)
	}
	...
	_ = os.Setenv("PATH", directory+string(os.PathListSeparator)+path)
}
