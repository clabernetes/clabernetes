---
title: Quickstart
description: Launch c9s and a sample network lab in a disposable KinD cluster.
icon: Rocket
---

The [repository](https://github.com/clabernetes/clabernetes) includes an automated workflow that creates a single-node [KinD](https://kind.sigs.k8s.io/) cluster, installs the
selected c9s Helm chart, configures MetalLB, and deploys a sample network topology: a Nokia SR Linux node connected to a Network Multitool node.

## Requirements

The `Makefile` provided in the repository automates all steps required to create the KinD cluster and deploy the sample network topology. You only need:

- `git`
- enough local CPU and memory for the KinD cluster and the sample nodes.
8GB of RAM and 4 CPUs are enough.

KinD, `kubectl`, Helm, yq, UV, and GitHub CLI are downloaded into `build/try-c9s/bin` at the
repository-pinned versions. Host executables with the same names are not used by this workflow.

## Clone the repository [step]

From the directory where you want to clone the repository, run:

```bash
git clone https://github.com/clabernetes/clabernetes
cd clabernetes
```

## Run the `try-c9s` workflow [step]

From the repository root, run:

```bash
make try-c9s
```

This creates the KinD cluster, installs c9s, configures the MetalLB load balancer, and deploys the sample topology.

A successful run ends with the following output:

```text
--> TRY-C9S: SR Linux SSH: ssh admin@<load-balancer-ip-1>
--> TRY-C9S: SR Linux gNMI: <load-balancer-ip-1>:57400
--> TRY-C9S: SR Linux NETCONF: <load-balancer-ip-1>:830
--> TRY-C9S: Multitool SSH: ssh admin@<load-balancer-ip-2>
--> TRY-C9S: clabernetes is ready to try
```

The exact load balancer address depends on the local cluster configuration.

## Connect to the nodes [step]

The previous step ends with the list of commands to use to connect to the two nodes in the sample network topology. You can always re-run `make try-c9s-print-access` from the repository root to print the access commands again.

<TryC9sDiagram />

The access commands show how to connect:

- with SSH/gNMI/NETCONF to the Nokia SR Linux node and
- with SSH to the Network Multitool node.

Connect to the SR Linux node with SSH and log in as the admin user with the password `NokiaSrl1!`.

## Test the datapath [step]

You can now test that the datapath is working by sending a ping from SR Linux `ethernet-1/1` interface to the Network Multitool `eth1` interface.

```
--{ running }--[  ]--
A:admin@srl1# ping network-instance default 192.0.2.1
Using network instance default
PING 192.0.2.1 (192.0.2.1) 56(84) bytes of data.
64 bytes from 192.0.2.1: icmp_seq=1 ttl=64 time=5.98 ms
```

## Explore the deployment

Once the datapath works, explore the deployment with the `kubectl` downloaded into `build/try-c9s/bin` at the repository root.

## Clean up

When you are done exploring c9s, remove the KinD cluster:

```bash
make try-c9s-clean
```

## Next steps

- [Understand the Nodes and Links API](/docs/concepts/nodes-and-links)
- [See what differs from containerlab](/docs/concepts/containerlab-differences)
- [Read the architecture overview](/docs/architecture)
- [Browse complete examples](/docs/examples)
