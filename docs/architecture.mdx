...
as an SR-SIM license at the same normalized destination, the Deployment renders that destination
once only when the ConfigMap, key, and mode agree. Conflicting attachments stop reconciliation
instead of silently selecting the first source.
