...
func TestWatchLinksClosesInitialListWatchGap(t *testing.T) {
	...
	localNodes := map[string]bool{"r1": true}

	initial, err := ListNodeTunnels(t.Context(), client, "default", localNodes)
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Setenv("LAUNCHER_GROUP_MEMBERS", "")

	ctx, cancel := context.WithCancel(t.Context())
	defer cancel()

	updates := make(chan []*Tunnel, 1)
