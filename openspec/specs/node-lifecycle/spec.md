...
component that owns the shared network namespace for application probes. Missing or ambiguous
component identity or network-namespace ownership MUST fail launcher discovery.
...
The Node spec SHALL describe files and other payload required to instantiate that network node, including supported URL- and ConfigMap-backed sources. Launcher Pod policy MUST NOT be used merely to associate a payload attachment with one Node. When Nodes share one launcher filesystem and declare the same destination for a shared payload, the controller SHALL render that destination as one Pod mount instead of producing duplicate Kubernetes mount paths.
...
#### Scenario: Group members share a license destination

- **WHEN** grouped Nodes reference a shared license payload at the same destination path
