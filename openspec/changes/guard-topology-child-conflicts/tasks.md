...
- [x] 2.1 Refactor Topology reconciliation to compile and render the complete LauncherProfile, Link, and Node set before mutating legacy or child resources.
- [x] 2.2 Implement uncached namespace/name occupancy checks for every desired LauncherProfile, Link, and Node, allowing only resources recognized as generated for the current Topology.
...
- [x] 2.5 Clear the conflict error and resume normal legacy cleanup, dependency ordering, drift correction, pruning, and aggregate status reconciliation after a conflict-free preflight.

## 3. Tests and documentation

- [x] 3.1 Add tests covering Node, Link, and LauncherProfile conflicts, including exact status formatting and deterministic ordering.
