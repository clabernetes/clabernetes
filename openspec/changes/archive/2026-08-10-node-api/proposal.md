...
- Map `labels` onto Kubernetes rather than dropping it: there is still no `spec.labels` on a Node, but node labels in a Topology `definition:` are inherited like `env`, copied to the emitted Node's `metadata.labels`, and propagated to the launcher Deployment and its pods — so they are selectable with kubectl, which docker labels on a node container never were. Labels Kubernetes would reject, labels in the `c9s.run/` namespace, and controller-owned identity/selector keys are omitted with a warning.
