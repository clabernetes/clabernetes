...
- **THEN** the workflow summary includes `make install C9S_VERSION=0.0.0-abc1234` and `make try-c9s C9S_VERSION=0.0.0-abc1234`
...
The installation workflows SHALL treat `C9S_VERSION=main` as exact chart version `0.0.0` in the development channel and SHALL accept exact unpublished versions matching `0.0.0-<sha>`. They SHALL read full source revision metadata from development charts, probe the exact chart and referenced images, and SHALL never classify either channel as latest stable.

#### Scenario: Install mutable main
- **WHEN** a user runs `make install C9S_VERSION=main`
...
- **WHEN** a user runs `make install C9S_VERSION=0.0.0-abc1234`
