...
Both public installation targets SHALL accept `C9S_VERSION=latest`, `C9S_VERSION=main`, an exact published version with or without a leading `v`, an exact unpublished `0.0.0-<sha>` version, `C9S_VERSION=local`, or `C9S_VERSION=select`. When `C9S_VERSION` is unset, the targets SHALL behave as `C9S_VERSION=latest` without prompting.

#### Scenario: Default latest installation
- **WHEN** a user runs either installation target without setting `C9S_VERSION`
...
- **WHEN** a user sets `C9S_VERSION=0.0.0-abc1234`
...
- **WHEN** `make install C9S_VERSION=local` targets a cluster verified as KinD
