...
- Support stable, development, and local selections through one common `VERSION` contract for both `make install` and `make try-c9s`, covering latest stable release, exact published release, mutable main, exact unpublished commit build, local checkout, and interactive selection.
